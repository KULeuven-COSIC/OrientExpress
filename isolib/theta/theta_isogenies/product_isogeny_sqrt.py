from isolib.theta.theta_isogenies.gluing_isogeny import GluingThetaIsogeny
from isolib.theta.theta_isogenies.isomorphism import SplittingIsomorphism
from isolib.theta.theta_isogenies.isogeny import ThetaIsogeny
from isolib.theta.theta_isogenies.product_isogeny import EllipticProductIsogeny
from isolib.theta.theta_isogenies.isogeny_sqrt import ThetaIsogeny4, ThetaIsogeny2
from isolib.theta.utilities.strategy import optimised_strategy
        ...
        for k in range(self.n - 2):
            ...
            if k == 0:
                try:
                    ...
                    print(f'Warning gluing: you probably encountered a product after {k} steps')
                    print(f'{self.n = }')
                    raise ValueError(f"product error: {k}")
            else:
                try:
                    phi = ThetaIsogeny(Th, Tp1, Tp2)
                except:
                    print(f'Warning: you probably encountered a product after {k} steps')
                    print(f'{self.n = }')
                    raise ValueError(f"product error: {k}")
                    # breakpoint()
            Th = phi.codomain()
